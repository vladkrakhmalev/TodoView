...
export const useResizeTask = ({ task, onResize }: IProps) => {
  ...
  const handleMouseMove = useCallback((e: globalThis.MouseEvent) => {
    ...
    const newHeight = Math.max(CELL_HEIGHT, Math.floor((resizeState.initialHeight + deltaY) / CELL_HEIGHT) * CELL_HEIGHT) - 4
  ...
  const handleMouseUp = useCallback(() => {
    ...
    const newDuration = Math.floor((taskElement.offsetHeight + 4) / CELL_HEIGHT) * 30
