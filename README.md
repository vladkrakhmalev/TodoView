...
- **`check-commit-msg`**:
  Проверяет сообщения коммитов на соответствие принятым в проекте правилам и конвенциям.
  **Команда**: `pnpm check-commit-msg`
...
- **`coverage/`**: Отчеты о покрытии кода тестами (не отслеживается Git)

  - Создается при выполнении команды `pnpm test:coverage`
...
- **`storybook-static/`**: Собранная статическая версия Storybook (не отслеживается Git)

  - Создается при выполнении команды `pnpm storybook:build` и содержит готовую к деплою документацию компонентов
